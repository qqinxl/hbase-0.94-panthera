 ...
 */
package org.apache.hadoop.hbase.dot.access;
...
import org.apache.hadoop.hbase.dot.DotException;
import org.apache.hadoop.hbase.dot.DotInvalidIOException;
import org.apache.hadoop.hbase.dot.DotUtil;
import org.apache.hadoop.hbase.dot.doc.Document;
import org.apache.hadoop.hbase.dot.doc.DocSchemaMissMatchException;
...
public class DataManipulationOps extends BaseRegionObserver {
  ...
  // filter => rowkey => KVs
  private ConcurrentMap<Filter, Map<String, Set<KeyValue>>> filterKVs = new ConcurrentHashMap<Filter, Map<String, Set<KeyValue>>>();
  ...
  /**
   * Get the serializer null format
   * 
   * @param conf
   * @return
   */
  private static String getNullFormat(Configuration conf) {
    return conf.get(DotConstants.SERIALIZATION_NUL_FORMAT,
  ...
  private Filter getFilterInstance(Filter flt) {
    return flt instanceof FilterWrapper ? ( (FilterWrapper) flt )
        .getFilterInstance() : flt;
  }
   ...
   * @throws IOException
   */
  static public byte[] getDocValue(String docName, Map<byte[], byte[]> fields,
      Pair<Object, String> schemaPair, Configuration conf) throws IOException {
    Document docObject = Document.createDoc(schemaPair.getSecond());
    ByteArrayOutputStream bao = new ByteArrayOutputStream();
    docObject.setNullFormat(getNullFormat(conf));
    // TODO choose the schema format (either file or string )
    docObject.initialize(schemaPair.getFirst(), (Object) null, bao);
  ...
  private void prepareGetOrScan(Object operator) throws IOException,
    ...
    Filter flt = getFilterInstance((Filter) clz.getMethod("getFilter").invoke(
        operator));
    if (flt != null) {
      Map<String, Set<KeyValue>> rows = new HashMap<String, Set<KeyValue>>();
      this.filterKVs.put(flt, rows);
      this.docObjectCache.put(flt, new TreeMap<byte[], Document>(
          Bytes.BYTES_COMPARATOR));
    }
  }
  ...
  public void postGet(final ObserverContext<RegionCoprocessorEnvironment> e,
    ...
    try {
      ...
      byte[] nullValue = getNullFormat(conf).getBytes();
  ...
  public boolean postScannerNext(
    ...
    while (iterator.hasNext()) {
      ...
      kvlist.flush();
      // remove the processed Result element
      iterator.remove();
    }
  ...
  @Override
  public boolean preFilterRow(ObserverContext<RegionCoprocessorEnvironment> e,
    ...
    HTableDescriptor desc = e.getEnvironment().getRegion().getTableDesc();
    if (!DotUtil.isDot(desc)) return false;
    // docRow -> (kv, index) , index is sorted in ASC
    Map<String, Pair<KeyValue, Integer>> returnList = new HashMap();

    List<KeyValue> toProcess = new ArrayList();
    for (int i = 0; i < kvs.size(); i++) {
      KeyValue kv = kvs.get(i);
      // LOG.info("kv in preFilterRow:  " + kv);
      List<KeyValue> newList = null;
      ...
      newList = new ArrayList<KeyValue>(filterKVs.get(filter).get(docRow));
      toProcess.addAll(newList);
      newList.clear();
    }
    ...
    if (!toDelete) {
      ...
      for (KeyValue kv : toProcess) {
        ...
        KeyValue kvCopy = new KeyValue(kv.getBuffer(), kv.getRowOffset(),
            ...
            kv.getTimestamp(), Type.codeToType(kv.getType()), kv.getBuffer(),
            kv.getValueOffset(), kv.getValueLength());
        String docRow = kvCopy.getKeyString();
        returnList.get(docRow).setSecond(null);
        Set kvsets = filterKVs.get(filter).get(docRow);
        kvsets.add(kv);
      }
  ...
  public boolean preFilterTransform(
      ObserverContext<RegionCoprocessorEnvironment> e, KeyValue kv,
    ...
    if (DotUtil.isDot(desc)) {
      ...
      Iterator iter = this.filterKVs.get(filter).get(docRow).iterator();
  ...
  public KeyValue postFilterGetNextKeyHint(
    ...
    if (DotUtil.isDot(desc)) {
      KeyValue newKV = null;
      byte[][] df = DotUtil.getDocAndField(returnedKV.getBuffer(),
          returnedKV.getQualifierOffset(), returnedKV.getQualifierLength());
  ...
  public boolean preFilterKeyValue(
    ...
    if (DotUtil.isDot(desc)) {
      ...
      String docRow = kv.getKeyString();
      Set<KeyValue> kvSets = this.filterKVs.get(filter).get(docRow);
      if (kvSets == null) {
        kvSets = new TreeSet<KeyValue>(new KeyValue.KVComparator());
        this.filterKVs.get(filter).put(docRow, kvSets);
      }
  ...
  private Document decodeDocValue(HColumnDescriptor coldef, KeyValue kv,
      Document docObject) throws IOException {
    if (docObject == null) {
  ...
  private enum ReturnCodePriority {
    ...
    INCLUDE(ReturnCode.INCLUDE), SKIP(ReturnCode.SKIP), NEXT_COL(
        ReturnCode.NEXT_COL), NEXT_ROW(ReturnCode.NEXT_ROW), SEEK_NEXT_USING_HINT(
  ...
  private ReturnCode getFilterKeyValueDecision(Set<byte[]> fieldList,
      KeyValue kv, Filter filter, Document docObject, ReturnCode result,
    ...
    for (byte[] field : fieldList) {
      byte[] newQualifier = DotUtil.combineDocAndField(qualifier, field);
      byte[] newValue = docObject.getValue(field);
      if (Bytes.equals(newValue, getNullFormat(conf).getBytes())  ) {

...
public interface RegionObserver extends Coprocessor {
  ...
  /**
   * Called before {@link org.apache.hadoop.hbase.filter.Filter#filterRow(List)}<br>
   * 
   * @param ctx
   * @param kvs
   * @param filter
   * @return if to bypass the filterRow(List)
   * @throws IOException
   */
  boolean preFilterRow(final ObserverContext<RegionCoprocessorEnvironment> ctx,
      List<KeyValue> kvs, Filter filter) throws IOException;

  /**
   * Called after {@link org.apache.hadoop.hbase.filter.Filter#filterRow(List)}<br>
   * @param ctx
   * @param kvs
   * @param filter
   * @throws IOException
   */
  void postFilterRow(final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final List<KeyValue> kvs, Filter filter) throws IOException;

  /**
   * Called before {@link org.apache.hadoop.hbase.filter.Filter#transform(KeyValue)}<br>
   * @param ctx
   * @param kv
   * @param returnedKV KeyValue[1]
   * @param filter
   * @return if to bypass the transform(KeyValue)
   * @throws IOException
   */
  boolean preFilterTransform(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final KeyValue kv, final KeyValue[] returnedKV, final Filter filter)
      throws IOException;

  /**
   * Called after {@link org.apache.hadoop.hbase.filter.Filter#transform(KeyValue)}<br>
   * @param ctx
   * @param kv
   * @param returnedKV
   * @param filter
   ...
   * @throws IOException
   */
  KeyValue postFilterTransform(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final KeyValue kv, final KeyValue returnedKV, final Filter filter)
      throws IOException;

  /**
   * Called before {@link org.apache.hadoop.hbase.filter.Filter#getNextKeyHint(KeyValue)}<br>
   * @param ctx
   * @param kv
   * @param returnedKV KeyValue[1]
   * @param filter
   * @return if to bypass the getNextKeyHint(KeyValue)
   * @throws IOException
   */
  boolean preFilterGetNextKeyHint(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final KeyValue kv, final KeyValue[] returnedKV, final Filter filter)
      throws IOException;

  /**
   * Called after {@link org.apache.hadoop.hbase.filter.Filter#getNextKeyHint(KeyValue)}<br>
   * @param ctx
   * @param kv
   * @param returnedKV
   * @param filter
   ...
   * @throws IOException
   */
  KeyValue postFilterGetNextKeyHint(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final KeyValue kv, final KeyValue returnedKV, final Filter filter)
      throws IOException;

  /**
   * Called before {@link org.apache.hadoop.hbase.filter.Filter#filterRowKey(byte[], int, int)}
   * @param ctx
   * @param buffer
   * @param offset
   * @param length
   ...
   * @throws IOException
   */
  boolean preFilterRowKey(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final byte[] buffer, final int offset, final int length,
      boolean result, final Filter filter) throws IOException;

  /**
   * Called after {@link org.apache.hadoop.hbase.filter.Filter#filterRowKey(byte[], int, int)}
   * @param ctx
   * @param buffer
   * @param offset
   * @param length
   ...
   * @throws IOException
   */
  boolean postFilterRowKey(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final byte[] buffer, final int offset, final int length,
      final boolean result, final Filter filter) throws IOException;

  /**
   * Called before {@link org.apache.hadoop.hbase.filter.Filter#filterKeyValue(KeyValue)}<br>
   * @param ctx
   * @param kv
   * @param r
   * @param filter
   ...
   * @throws IOException
   */
  boolean preFilterKeyValue(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final KeyValue kv, ReturnCode r, final Filter filter)
      throws IOException;

  /**
   * Called after {@link org.apache.hadoop.hbase.filter.Filter#filterKeyValue(KeyValue)}<br>
   * @param ctx
   * @param kv
   * @param r
   * @param filter
   ...
   * @throws IOException
   */
  ReturnCode postFilterKeyValue(
      final ObserverContext<RegionCoprocessorEnvironment> ctx,
      final KeyValue kv, final ReturnCode r, final Filter filter)
      throws IOException;

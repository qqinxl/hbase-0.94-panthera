...
public class RegionCoprocessorHost
   ...
   * @throws IOException
   */
  public boolean preFilterRow(final List<KeyValue> kvs, final Filter filter)
      throws IOException {
    boolean bypass = false;
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          ( (RegionObserver) env.getInstance() ).preFilterRow(ctx, kvs, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
        bypass |= ctx.shouldBypass();
   ...
   * @throws IOException
   */
  public void postFilterRow(final List<KeyValue> kvs, final Filter filter)
      throws IOException {
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
   ...
   * @throws IOException
   */
  public boolean preFilterTransform(final KeyValue kv,
      final KeyValue[] returnedKV, final Filter filter) throws IOException {
    boolean bypass = false;
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          ( (RegionObserver) env.getInstance() ).preFilterTransform(ctx, kv,
              returnedKV, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
   ...
   * @throws IOException
   */
  public KeyValue postFilterTransform(final KeyValue kv, KeyValue returnedKV,
    ...
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          returnedKV = ( (RegionObserver) env.getInstance() )
              .postFilterTransform(ctx, kv, returnedKV, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
        if (ctx.shouldComplete()) {
  ...
  public boolean preFilterGetNextKeyHint(final KeyValue kv,
      final KeyValue[] returnedKV, final Filter filter) throws IOException {
    boolean bypass = false;
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          ( (RegionObserver) env.getInstance() ).preFilterGetNextKeyHint(ctx,
              kv, returnedKV, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
        bypass |= ctx.shouldBypass();
   ...
   * @throws IOException
   */
  public KeyValue postFilterGetNextKeyHint(final KeyValue kv,
      KeyValue returnedKV, final Filter filter) throws IOException {
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          returnedKV = ( (RegionObserver) env.getInstance() )
              .postFilterGetNextKeyHint(ctx, kv, returnedKV, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
        if (ctx.shouldComplete()) {
   ...
   * @throws IOException
   */
  public boolean preFilterRowKey(final byte[] buffer, final int offset,
      final int length, final boolean result, final Filter filter)
      throws IOException {
    boolean bypass = false;
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          ( (RegionObserver) env.getInstance() ).preFilterRowKey(ctx, buffer,
              offset, length, result, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
        bypass |= ctx.shouldBypass();
   ...
   * @throws IOException
   */
  public boolean postFilterRowKey(final byte[] buffer, final int offset,
      final int length, boolean result, final Filter filter) throws IOException {
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          result = ( (RegionObserver) env.getInstance() ).preFilterRowKey(ctx,
              buffer, offset, length, result, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
        if (ctx.shouldComplete()) {
   ...
   * @throws IOException
   */
  public boolean preFilterKeyValue(final KeyValue kv, final ReturnCode r,
      final Filter filter) throws IOException {
    boolean bypass = false;
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          ( (RegionObserver) env.getInstance() ).preFilterKeyValue(ctx, kv, r,
              filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
   ...
   * @throws IOException
   */
  public ReturnCode postFilterKeyValue(final KeyValue kv, ReturnCode r,
      final Filter filter) throws IOException {
    ObserverContext<RegionCoprocessorEnvironment> ctx = null;
    for (RegionEnvironment env : coprocessors) {
      if (env.getInstance() instanceof RegionObserver) {
        ctx = ObserverContext.createAndPrepare(env, ctx);
        try {
          r = ( (RegionObserver) env.getInstance() ).postFilterKeyValue(ctx,
              kv, r, filter);
        } catch (Throwable e) {
          handleCoprocessorThrowable(env, e);
        }
